...
この拡張機能は､Redmineにちょっとした秩序とちょっとしたモダンUIをもたらし､PMの離職率を下げることを目的としている｡
...
- **リアルタイム更新** - 他のユーザーの変更を即座に反映
- **フィルタリング** - Version、担当者、ステータスで絞り込み
- **一括操作** - 複数チケットを選択して一括編集
...
- **階層制約チェック** - 不正な親子関係の設定を防ぐ警告表示
- **日付整合性チェック** - 親チケットと子チケットの期日矛盾を検出
- **Version連動表示** - 関連するVersionとリリース予定日を強調表示
- **子チケット一括作成** - User StoryからTask/Bug/Testを素早く追加

慣れたUIはそのままに、必要な機能だけを追加している。
