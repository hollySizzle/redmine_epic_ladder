...
Rails.application.config.after_initialize do
  next unless defined?(RedmineEpicLadder)
  ...
  if source_dir.exist?
    begin
      ...
      Rails.logger.info "[EpicLadder] Assets deployed: #{deployed_files} files from #{source_dir} to #{dest_dir}"
    rescue => e
      Rails.logger.error "[EpicLadder] Failed to deploy assets: #{e.message}"
      Rails.logger.error e.backtrace.join("\n")
    end
  else
    Rails.logger.warn "[EpicLadder] Build directory not found: #{source_dir}"
    Rails.logger.warn "[EpicLadder] Run 'npm run build:prod' in plugin directory to build assets"
  end
end
