...
RSpec.describe EpicLadder::McpToolHintsController, type: :controller do
  routes { RedmineApp::Application.routes }

  let(:project) { create(:project) }
  let(:user) { create(:user, admin: true) }
  let(:role) { create(:role, permissions: [:view_epic_ladder, :manage_epic_ladder]) }

  before do
    # Enable epic_ladder module for project
  ...
  describe 'PATCH #update' do
    context 'with valid params' do
      it 'creates new hints' do
        ...
        hint1 = EpicLadder::McpToolHint.find_by(project: project, tool_key: 'create_task')
        expect(hint1.enabled).to be true
        expect(hint1.hint_text).to eq('タスク作成時のヒント')

        hint2 = EpicLadder::McpToolHint.find_by(project: project, tool_key: 'add_issue_comment')
    ...
    context 'with invalid tool_key' do
      it 'ignores invalid tool keys' do
        ...
        expect(response).to redirect_to(settings_project_path(project, tab: 'epic_ladder'))
        expect(EpicLadder::McpToolHint.find_by(project: project, tool_key: 'invalid_tool')).to be_nil
        expect(EpicLadder::McpToolHint.find_by(project: project, tool_key: 'create_task')).to be_present
      end
    end

    context 'without permission' do
      let(:user) { create(:user, admin: false) } # non-admin user
      let(:role) { create(:role, permissions: [:view_epic_ladder]) } # no manage_epic_ladder
      ...
      it 'denies access (403 or redirect)' do
        ...
        expect(EpicLadder::McpToolHint.find_by(project: project, tool_key: 'create_task')).to be_nil
      end
    end

    context 'with all modifying tools' do
      it 'can save hints for all tool types' do
        all_hints = EpicLadder::McpToolHint::MODIFYING_TOOLS.each_with_object({}) do |tool, hash|
        ...
        EpicLadder::McpToolHint::MODIFYING_TOOLS.each do |tool|
          hint = EpicLadder::McpToolHint.find_by(project: project, tool_key: tool)
