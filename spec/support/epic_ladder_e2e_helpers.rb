...
module EpicLadderE2EHelpers
  # プロジェクトとトラッカーのセットアップ
  def setup_epic_ladder_project(identifier: 'test-project', name: nil)
    ...
    # epic_ladderモジュール有効化
    project.enabled_modules.create!(name: 'epic_ladder') unless project.module_enabled?('epic_ladder')
  ...
  # 一般ユーザー作成と権限設定
  def setup_user_with_permissions(project, login: 'test_user', permissions: [:view_issues, :add_issues, :edit_issues, :view_epic_ladder, :manage_epic_ladder])
  ...
  def goto_kanban(project)
    @playwright_page.goto("/projects/#{project.identifier}/epic_ladder", timeout: 30000)
    @playwright_page.wait_for_load_state('networkidle', timeout: 10000) rescue nil
...
RSpec.configure do |config|
  config.include EpicLadderE2EHelpers, type: :system
end
