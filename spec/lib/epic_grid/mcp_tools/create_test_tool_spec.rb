...
RSpec.describe EpicGrid::McpTools::CreateTestTool, type: :model do
  ...
  describe '.call' do
    ...
    context 'with valid parameters' do
      it 'creates a Test successfully' do
        ...
        expect(test.tracker.name).to eq(EpicGrid::TrackerHierarchy.tracker_names[:test])
    ...
    context 'with invalid parameters' do
      ...
      it 'returns error when user lacks permission' do
        ...
        response_text = JSON.parse(result.content.first[:text])
        expect(response_text['success']).to be false
        expect(response_text['error']).to include('Test作成権限がありません')
      end
      ...
      it 'returns error when Test tracker not configured' do
        ...
        response_text = JSON.parse(result.content.first[:text])
        expect(response_text['success']).to be false
        expect(response_text['error']).to include('Testトラッカーが設定されていません')
      end
    end
  end
