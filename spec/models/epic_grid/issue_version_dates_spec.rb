...
RSpec.describe 'IssueExtensions#epic_grid_apply_version_dates!', type: :model do
  ...
  describe '#epic_grid_apply_version_dates!' do
    context 'バージョンが設定されていない場合' do
      it 'falseを返し、日付は変更されない' do
        ...
        result = user_story.epic_grid_apply_version_dates!
    ...
    context '親issueと子issue（task、test、bug）がある場合' do
      ...
      it '親issueと全ての子issueに同じ日付が設定される' do
        # 日付計算を事前に行う
        dates = EpicGrid::VersionDateManager.update_dates_for_version_change(user_story, version_b)
    ...
    context '最も早いバージョンに移動する場合' do
      ...
      it '親と子issueの両方の開始日・終了日が同じバージョンの期日になる' do
        # 日付計算を事前に行う
        dates = EpicGrid::VersionDateManager.update_dates_for_version_change(user_story, version_a)
