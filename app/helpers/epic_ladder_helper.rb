...
module EpicLadderHelper
  ...
  def epic_ladder_asset_path(asset_name)
    ...
    unless File.exist?(manifest_path)
      return "#{Redmine::Utils.relative_url_root}/plugin_assets/redmine_epic_ladder/#{asset_name}"
    end
    ...
    begin
      ...
      if hashed_name
        ...
        if hashed_name.start_with?('/plugin_assets/redmine_epic_ladder/')
        ...
        "#{Redmine::Utils.relative_url_root}/plugin_assets/redmine_epic_ladder/#{asset_name}"
      end
    rescue JSON::ParserError, Errno::ENOENT => e
      Rails.logger.warn "[EpicLadder] Failed to load asset manifest: #{e.message}"
      "#{Redmine::Utils.relative_url_root}/plugin_assets/redmine_epic_ladder/#{asset_name}"
    end
  end
end
