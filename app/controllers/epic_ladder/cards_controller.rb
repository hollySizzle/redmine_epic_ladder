...
module EpicLadder
  ...
  class CardsController < BaseApiController
    ...
    def index
      ...
      kanban_data = EpicLadder::KanbanDataBuilder.new(@project, User.current, filter_params).build
    ...
    def create
      ...
      # Featureトラッカー取得
      feature_tracker = Tracker.find_by(name: EpicLadder::TrackerHierarchy.tracker_names[:feature])
      ...
      # MSW準拠のレスポンス構築
      updated_entities = {
        features: { feature.id.to_s => feature.epic_ladder_as_normalized_json }
      }
      updated_entities[:epics] = { parent_epic.id.to_s => parent_epic.epic_ladder_as_normalized_json } if parent_epic
      ...
      if feature.parent_id && parent_epic
        ...
        grid_index = @project.epic_ladder_index
      ...
      render_normalized_success(
        created_entity: feature.epic_ladder_as_normalized_json,
    ...
    def update
      ...
      result = EpicLadder::FeatureUpdateService.execute(
        feature: feature,
    ...
    def show
      ...
      render_success({
        ...
        user_stories: serialize_user_stories_with_children(feature.children.joins(:tracker).where(trackers: { name: EpicLadder::TrackerHierarchy.tracker_names[:user_story] })),
        relationships: serialize_relationships(feature),
        activity_timeline: build_activity_timeline(feature),
        validation_status: EpicLadder::ValidationGuardService.validate_feature_readiness(feature)
      })
    rescue ActiveRecord::RecordNotFound
      render_error('指定されたFeatureが見つかりません', :not_found)
    end
    ...
    def create_user_story
      ...
      user_story_tracker = Tracker.find_by(name: EpicLadder::TrackerHierarchy.tracker_names[:user_story])
      ...
      dates = if target_version_id
        ...
        EpicLadder::VersionDateManager.update_dates_for_version_change(temp_issue, version)
      end
      ...
      # MSW準拠のレスポンス構築
      render_normalized_success(
        created_entity: user_story.epic_ladder_as_normalized_json,
        updated_entities: {
          features: { feature.id.to_s => feature.epic_ladder_as_normalized_json },
          user_stories: { user_story.id.to_s => user_story.epic_ladder_as_normalized_json }
        },
        grid_updates: grid_updates,
        status: :created
      )
    ...
    def update_user_story
      ...
      result = EpicLadder::UserStoryUpdateService.execute(
    ...
    def destroy_user_story
      ...
      result = EpicLadder::UserStoryDeletionService.execute(
        user_story: user_story,
        user: User.current
      )
    ...
    def create_task
      ...
      task_tracker = Tracker.find_by(name: EpicLadder::TrackerHierarchy.tracker_names[:task])
      ...
      # MSW準拠のレスポンス構築
      render_normalized_success(
        created_entity: task.epic_ladder_as_normalized_json,
        updated_entities: {
          user_stories: { user_story.id.to_s => user_story.epic_ladder_as_normalized_json },
          tasks: { task.id.to_s => task.epic_ladder_as_normalized_json }
        },
        status: :created
      )
    rescue ActiveRecord::RecordNotFound
    ...
    def create_test
      ...
      user_story = Issue.find(user_story_id)
      test_tracker = Tracker.find_by(name: EpicLadder::TrackerHierarchy.tracker_names[:test])
      ...
      # 正規化APIレスポンス形式
      render_normalized_success(
        created_entity: test.epic_ladder_as_normalized_json,
        updated_entities: {
          tests: { test.id.to_s => test.epic_ladder_as_normalized_json },
          user_stories: { user_story.id.to_s => user_story.reload.epic_ladder_as_normalized_json }
        },
        status: :created
      )
    rescue ActiveRecord::RecordNotFound
    ...
    def create_bug
      ...
      user_story = Issue.find(user_story_id)
      bug_tracker = Tracker.find_by(name: EpicLadder::TrackerHierarchy.tracker_names[:bug])
      ...
      # 正規化APIレスポンス形式
      render_normalized_success(
        created_entity: bug.epic_ladder_as_normalized_json,
        updated_entities: {
          bugs: { bug.id.to_s => bug.epic_ladder_as_normalized_json },
          user_stories: { user_story.id.to_s => user_story.reload.epic_ladder_as_normalized_json }
        },
        status: :created
      )
    ...
    def update_item
      ...
      result = EpicLadder::ItemUpdateService.execute(
        item: item,
    ...
    def destroy_item
      ...
      result = EpicLadder::ItemDeletionService.execute(
        item: item,
        user: User.current
      )
    ...
    def bulk_update_user_stories
      ...
      result = EpicLadder::BulkUpdateService.execute(
    ...
    def serialize_issue_with_children(issue)
      ...
      feature_tracker_name = EpicLadder::TrackerHierarchy.tracker_names[:feature]
          ...
          user_stories: user_stories.map do |us|
            {
              ...
              tasks: serialize_children_by_tracker(us, EpicLadder::TrackerHierarchy.tracker_names[:task]),
              tests: serialize_children_by_tracker(us, EpicLadder::TrackerHierarchy.tracker_names[:test]),
              bugs: serialize_children_by_tracker(us, EpicLadder::TrackerHierarchy.tracker_names[:bug])
            }
          end
        })
      else
