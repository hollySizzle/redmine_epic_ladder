...
module EpicLadder
  module IssueExtensions
    ...
    class_methods do
      def epic_ladder_tracker_cache
        @epic_ladder_tracker_cache ||= {}
      end

      def epic_ladder_find_tracker_by_name(name)
        epic_ladder_tracker_cache[name] ||= Tracker.find_by(name: name)
      end

      def epic_ladder_clear_tracker_cache!
        @epic_ladder_tracker_cache = {}
      end
    end
    ...
    def epic_ladder_move_to_cell(target_epic_id, target_version_id)
      transaction do
        # 親とバージョンを更新
        ...
        # 子要素にバージョンを伝播
        epic_ladder_propagate_version_to_children(target_version_id)
      end
    end

    # 子孫全てにバージョンを伝播
    # @param version_id [Integer, nil] 伝播するバージョンID
    def epic_ladder_propagate_version_to_children(version_id = nil)
    ...
    def epic_ladder_apply_version_dates!
      ...
      dates = EpicLadder::VersionDateManager.update_dates_for_version_change(self, fixed_version)
      if dates
    ...
    def epic_ladder_as_normalized_json
      base = {
        id: id.to_s,
        subject: subject,
      ...
      # トラッカー別に異なるフィールドを追加
      case tracker.name
      when EpicLadder::TrackerHierarchy.tracker_names[:epic]
        base.merge(
      ...
      when EpicLadder::TrackerHierarchy.tracker_names[:feature]
        base.merge(
          title: subject,
      ...
      when EpicLadder::TrackerHierarchy.tracker_names[:user_story]
        # Trackerを事前にキャッシュから取得（N+1対策）
        task_tracker = Issue.epic_ladder_find_tracker_by_name(EpicLadder::TrackerHierarchy.tracker_names[:task])
        test_tracker = Issue.epic_ladder_find_tracker_by_name(EpicLadder::TrackerHierarchy.tracker_names[:test])
        bug_tracker = Issue.epic_ladder_find_tracker_by_name(EpicLadder::TrackerHierarchy.tracker_names[:bug])
      ...
      when EpicLadder::TrackerHierarchy.tracker_names[:task]
        base.merge(
          title: subject,
      ...
      when EpicLadder::TrackerHierarchy.tracker_names[:test]
        base.merge(
          title: subject,
          parent_user_story_id: parent_id&.to_s,
          test_result: 'pending',
      ...
      when EpicLadder::TrackerHierarchy.tracker_names[:bug]
        base.merge(
          title: subject,
          parent_user_story_id: parent_id&.to_s,
          severity: 'minor',
    ...
    def epic_ladder_build_issue_json
      {
        id: id,
        subject: subject,
        ...
        parent_id: parent_id,
        hierarchy_level: epic_ladder_determine_hierarchy_level,
        created_on: created_on.iso8601,
        updated_on: updated_on.iso8601,
        column: epic_ladder_determine_column_for_status,
        epic: epic_ladder_find_epic_name
      }
    end

    # トラッカー階層レベルを判定
    # @return [Integer] 階層レベル（1: Epic, 2: Feature, 3: UserStory, 4: Task/Test/Bug）
    def epic_ladder_determine_hierarchy_level
    ...
    def epic_ladder_determine_column_for_status
      case status.name
      when 'New', 'Open'
        'todo'
    ...
    def epic_ladder_find_epic_name
    ...
    def epic_ladder_calculate_epic_statistics
      features = children

      {
        total_features: features.count,
        completed_features: features.joins(:status).where(issue_statuses: { is_closed: true }).count,
        completion_ratio: epic_ladder_calculate_completion_ratio(features)
      }
    end

    # Feature完了率を計算
    # @return [Float] 完了率（0-100）
    def epic_ladder_calculate_feature_completion
      user_stories = children.joins(:tracker).where(
        trackers: { name: EpicLadder::TrackerHierarchy.tracker_names[:user_story] }
      )
    ...
    # UserStory完了率を計算
    # @return [Float] 完了率（0-100）
    def epic_ladder_calculate_user_story_completion
    ...
    # @return [Float] 完了率（0-100）
    def epic_ladder_calculate_completion_ratio(items)
    ...
    # Featureリスク評価
    # @return [Hash] リスク評価結果
    def epic_ladder_assess_feature_risk
      risk_factors = []
      ...
      {
        level: epic_ladder_calculate_risk_level(risk_factors),
    ...
    def epic_ladder_calculate_risk_level(factors)
      case factors.count
      when 0
        'low'
      when 1
