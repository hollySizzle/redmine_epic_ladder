...
module EpicGrid
  ...
  #
  # @example
  #   hint = EpicGrid::McpToolHint.for_tool(project, 'create_task')
  #   hint.hint_text = 'レビュー依頼時は@hollyをメンション'
  #   hint.save!
  #
  # @example ツールのdescriptionにヒントを付与
  #   description = EpicGrid::McpToolHint.build_description(
  #     project,
  #     'create_task',
  ...
  class McpToolHint < ActiveRecord::Base
    self.table_name = 'epic_grid_mcp_tool_hints'
    ...
    # @return [EpicGrid::McpToolHint] ヒント設定オブジェクト
    ...
    def tool_display_name
      I18n.t("epic_grid.mcp_tools.#{tool_key}", default: tool_key.titleize)
    end
  end
end
