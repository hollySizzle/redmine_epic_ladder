...
namespace :redmine_epic_grid do
  desc 'Build frontend assets (requires npm)'
  task :build do
    plugin_dir = 'plugins/redmine_epic_grid'
  ...
  task :deploy do
    source = Rails.root.join('plugins', 'redmine_epic_grid', 'assets', 'build')
    dest = Rails.public_path.join('plugin_assets', 'redmine_epic_grid')

    unless Dir.exist?(source)
      puts "❌ Build directory not found: #{source}"
      puts "   Run 'rake redmine_epic_grid:build' first (requires npm)"
  ...
  task :status do
    ...
    if !Dir.exist?(source)
      puts "  ⚠️  Run 'rake redmine_epic_grid:build' to create assets (requires npm)"
    elsif !Dir.exist?(dest)
      puts "  ⚠️  Run 'rake redmine_epic_grid:deploy' to deploy assets"
      ...
      if dest_mtime && source_mtime > dest_mtime
        ...
        puts "      Run 'rake redmine_epic_grid:deploy' or restart Rails"
      else
